// VENDORED + PATCHED copy of swift-bundler's GenericWindowsBundler.swift, pinned to commit
// 4ad3f14f0b4c292f5bb57105b834be7f321c4f05 (github.com/stackotter/swift-bundler).
//
// Only change from upstream: `enumerateDynamicLibraryDependencies` discovers a binary's DLL
// imports with `llvm-readobj --needed-libs` instead of `dumpbin /DEPENDENTS`. dumpbin
// (link.exe /dump) fails with `LNK1106: cannot seek to 0x...` on this app's large Swift .exe;
// llvm-readobj (same toolchain) parses it fine. See the CI workflow's "Build swift-bundler"
// step, which copies this file over the pinned checkout before building.
import Version
import Foundation
import Ico
import ImageFormats
...
enum GenericWindowsBundler: Bundler {
  ...
  static func enumerateDynamicLibraryDependencies(
    ...
    let output: String
    do {
      // Use `llvm-readobj` (ships in the Swift toolchain's bin dir) instead of
      // `dumpbin`. dumpbin (link.exe /dump) fails with `LNK1106: cannot seek to
      // 0x...` on large Swift executables, whereas llvm-readobj parses them
      // fine. `--needed-libs` prints the imported DLLs, one per line, between a
      // "NeededLibraries [" heading and a closing "]".
      output = try await Process.create(
        "llvm-readobj",
        arguments: ["--needed-libs", module.path],
    ...
    guard
      let headingIndex = lines.firstIndex(where: {
        $0.trimmingCharacters(in: .whitespaces) == "NeededLibraries ["
      })
    else {
      let message = ErrorMessage.failedToParseDumpbinOutput(
        output: output,
        message: "Couldn't find NeededLibraries section"
      )
      throw Error(message)
    }
    ...
    guard
      ...
      let message = ErrorMessage.failedToParseDumpbinOutput(
        output: output,
        message: "Couldn't find end of NeededLibraries section"
      )
      throw Error(message)
    }
